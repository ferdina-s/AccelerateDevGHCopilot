...
public class ConsoleApp
{
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        if (onLoan == null || onLoan.Count < bookItems.Count)
        {
            Console.WriteLine($"\"{book.Title}\" is available for loan.");
        }
        else
        {
            var earliestDue = onLoan.Min(l => l.DueDate);
            Console.WriteLine($"\"{book.Title}\" is on loan to another patron. The return due date is {earliestDue}.");
        }
